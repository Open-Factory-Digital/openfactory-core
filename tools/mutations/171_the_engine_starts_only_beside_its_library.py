...
THREE CLAIMS:
  ...
  2. **`up` names what is missing** — the library when the binary is there, both when neither is —
     and does not claim the panel works, because its page imports the library too.
...
MUTATIONS = [
    ...
    ("the closing line says the panel works on an install whose page cannot load", CLI,
     '        panel = "the panel works, " if host.the_client() else ""\n',
     '        panel = "the panel works, "\n'),
