...
MUTATIONS = [
    ...
    ("a preview may plant any cookie, a Domain one or one named like the panel's", APP,
    ...
    ("a Domain cookie from a preview passes when its name is its own", APP,
     ...
     "    return name.strip() not in ours"),
    ("a credential cookie that arrives twice is taken, last-wins", APP,
     "    return request.cookies.get(\"openfactory_token\", \"\") if seen == 1 else \"\"",
     "    return request.cookies.get(\"openfactory_token\", \"\")"),
    ("the page adopts the first credential cookie it finds", "openfactory/api/panel.html",
     "return all.length===1?decodeURIComponent(all[0][1]):\"\"}",
     "return all.length>=1?decodeURIComponent(all[0][1]):\"\"}"),
    ("the panel may be framed", APP,
     "    response.headers.setdefault(\"content-security-policy\", \"frame-ancestors 'none'\")\n",
     ""),
    ("the proxy target is whatever the record names", PREVIEW,
