...
documents said "{_STALE} decision records" over 41 ADRs for as long as they did, because
...
#: HOW MANY DECISION RECORDS THERE ARE, counted rather than typed. These anchors were literals and
#: rotted the moment ADR-0044 arrived from main — a plan that refuses to start is a plan nobody
#: runs, and this number moves every time anybody adds a record.
_COUNT = len(list((pathlib.Path(__file__).resolve().parents[2] / "docs" / "adr")
...
MUTATIONS = [
    ...
    ("CONTRIBUTING keeps the old count — the page a contributor reads first",
     "CONTRIBUTING.md",
     f"**why** — {_COUNT} decision records.",
     f"**why** — {_STALE} decision records."),

    ("the documentation map keeps the old count",
     "docs/README.md",
     f"why it is built this way — {_COUNT} decision records",
     f"why it is built this way — {_STALE} decision records"),
