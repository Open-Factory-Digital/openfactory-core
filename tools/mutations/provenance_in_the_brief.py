...
THREE CLAIMS:
  ...
  1. **The rule is at the top**, before the first byte of anybody else's text, and it names what a
     data block cannot do rather than asking for care.
  2. **Every section declares its kind**, and what a stranger can write — every field of the card
     — is always the DATA kind.
  3. **What binds, binds**: this project's own standing documents and a person's own answer are
...
MUTATIONS = [
    ...
    ("the brief opens on the ticket again, so the rule arrives after the text it is about", BASE,
     ...
     '    parts = [f"# Ticket {t.id}: {t.title}"]', TEST),
    ...
    ("the card's own words lose their kind, and sit beside the constitution unlabelled", BASE,
     ...
     '    parts += ["", "## The card", "", "### Objective", t.objective]', TEST),
