...
class ContainerSandbox(SandboxAdapter):
    ...
    def keep_for_preview(self, *, project: str, card: str, command: str, port: int,
                         env_names: tuple[str, ...] | list[str], network: str, hours: int,
                         pr_url: str = "") -> tuple[preview.Preview | None, str]:
        """Freeze this validated box and start it again as the card's preview (ADR-0050).

        Returns `(preview, "")`, or `(None, why)` when there is none — never raises: a preview
        that could not start is a sentence on the card, never a failed job.

        THE SAME BOX, NOT A REBUILD (D2), AND NOT THE SAME PROCESS SPACE EITHER. What was tested
        is this container's filesystem plus the checkout mounted at /workspace, so both are kept:
        the filesystem by `docker commit`, the checkout by handing its ownership to the preview.
        What is NOT kept is the container itself, because it is tainted by construction — its
        environment was set at `docker run` with the harness token and every `box.env` credential,
        and any process inside it can read PID 1's environment from /proc whatever the process's
        own environment says. So the preview is a NEW container from the frozen image, with those
        names overwritten to empty in the image's configuration, the harnesses' state removed from
        HOME before the freeze, and only the registry's `box.preview_env` passed in.

        Its own name and its own label (`preview.container_name`), so the next repair of this
        card — which removes whatever wears the JOB box's name — cannot take it down."""
        bad = [v for v in env_names if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", v or "")]
        if bad:
            return None, (f"box.preview_env entries must be environment variable NAMES, got "
                          f"{bad!r}")
        if not re.fullmatch(r"[a-zA-Z0-9][a-zA-Z0-9_.-]*", network or ""):
            return None, f"{network!r} is not a docker network name"
        if not (self._container and self._host_clone):
            return None, "the box is already gone"
        name = preview.container_name(project, card)
        image = preview.image_name(project, card)
        # ONE PREVIEW PER CARD: a later run of the same card replaces the earlier one.
        stop_preview(project, card, why="replaced by a newer run of the same card", record=False)

        scrub = " ".join(f'"$HOME/{rel}"' for rel in HARNESS_STATE)
        rc, out = _host(["docker", "exec", self._container, "sh", "-c", f"rm -rf {scrub}"])
        if rc != 0:
            return None, f"could not clear the harness's state from the box ({out.strip()[:160]})"
        changes: list[str] = []
        for var in dict.fromkeys((*_AUTH_ENV_VARS, *self.extra_env)):
            changes += ["--change", f"ENV {var}="]
        rc, out = _host(["docker", "commit", *changes, self._container, image], timeout=600)
        if rc != 0:
            return None, f"could not freeze the validated box ({out.strip()[:160]})"

        expires = int(time.time()) + max(1, int(hours)) * 3600
        run_cmd = [
            "docker", "run", "-d", "--name", name,
            "--label", preview.LABEL,
            "--label", f"{preview.LABEL_PROJECT}={project}",
            "--label", f"{preview.LABEL_CARD}={card}",
            "--label", f"{preview.LABEL_EXPIRES}={expires}",
            "--label", f"{preview.LABEL_CLONE}={self._host_clone}",
            "--label", f"{preview.LABEL_PR}={pr_url}",
            "--cpus", self.cpus, "--memory", self.memory,
            "--network", network,
            "-v", f"{self._host_clone}:{_WORKDIR}", "-w", _WORKDIR,
            "-e", f"PORT={int(port)}", "-e", "HOST=0.0.0.0",
            "--entrypoint", "sh",
        ]
        for var in dict.fromkeys(env_names):
            if os.environ.get(var):
                run_cmd += ["-e", var]  # value inherited from the daemon client env
        run_cmd += [image, "-c", command]
        rc, out = _host(run_cmd)
        if rc != 0:
            _host(["docker", "rm", "-f", name])
            _host(["docker", "image", "rm", "-f", image])
            return None, (f"could not start the preview (docker run exited {rc}): "
                          f"{out.strip()[:200]}")

        p = preview.Preview(project=project, card=str(card),
                            label=preview.host_label(project, card), container=name,
                            port=int(port), expires_at=expires, pr_url=pr_url)
        # THE CHECKOUT NOW BELONGS TO THE PREVIEW: `cleanup` removes the job's container and must
        # leave the tree the preview is serving. `stop_preview` removes it when the preview ends.
        self._host_clone = None
        preview.record_live(p)
        return p, ""


#: What the harnesses keep in the box's HOME — removed before a validated box is frozen for a
#: preview (ADR-0050 D6), because the frozen image is what agent-written code then runs in. A
#: session, a transcript or a cached login there is the factory's, not the application's.
HARNESS_STATE = (".claude", ".claude.json", ".codex", ".kimi", ".config/opencode",
                 ".local/share/opencode")


def _clone_is_ours(path: str) -> bool:
    """Whether a path read back from a preview's label may be deleted.

    A label is metadata anybody with the daemon can write, so the path is checked against what
    `prepare` itself makes: a directory named `openfactory-…` directly under the work directory
    (or the temp directory when none is configured). Anything else is left where it is."""
    if not path:
        return False
    p = Path(path)
    work_dir = (os.environ.get("OPENFACTORY_WORK_DIR") or "").strip() or tempfile.gettempdir()
    try:
        return (p.name.startswith("openfactory-") and p.parent.resolve() == Path(work_dir).resolve()
                and p.is_dir() and not p.is_symlink())
    except OSError:
        return False


def stop_preview(project: str, card: str, *, why: str, record: bool = True) -> bool:
    """Take one card's preview down: its container, its frozen image and its checkout. Idempotent;
    True when there was one to take down."""
    name = preview.container_name(project, card)
    rc, out = _host(["docker", "inspect", "--format",
                     '{{index .Config.Labels "' + preview.LABEL_CLONE + '"}}', name])
    if rc != 0:
        return False
    clone = out.strip()
    _host(["docker", "rm", "-f", name])
    _host(["docker", "image", "rm", "-f", preview.image_name(project, card)])
    if _clone_is_ours(clone):
        shutil.rmtree(clone, ignore_errors=True)
    elif clone:
        log.warning("the preview %s named %r as its checkout, which is not one this worker made — "
                    "left in place", name, clone)
    if record:
        preview.record_ended(project, card, why)
    log.info("OPENFACTORY_PREVIEW_STOPPED %s #%s — %s", project, card, why)
    return True


def reap_previews(*, pr_status: Callable[[str, str], str] | None = None,
                  now: float | None = None) -> list[str]:
    """End every preview that should not be up any more (ADR-0050 D5), and say which.

    A preview ends when its time is up, when its pull request merged or was closed, or when its
    `serve:` command stopped (a dead preview advertised as live is a link to an error). Read from
    the daemon, not from the records: the daemon is what is actually running, and a record with
    no container behind it is what `stop_preview` corrects on the panel's side.

    `pr_status(project, pr_url)` answers "merged" | "closed" | "open"; it may raise, and a PR
    whose state could not be read keeps its preview until the clock ends it — never the reverse."""
    now = time.time() if now is None else now
    fmt = "|".join(["{{.Names}}", "{{.State}}",
                    *('{{.Label "' + k + '"}}' for k in (preview.LABEL_PROJECT, preview.LABEL_CARD,
                                                         preview.LABEL_EXPIRES, preview.LABEL_PR))])
    rc, out = _host(["docker", "ps", "-a", "--filter", f"label={preview.LABEL}", "--format", fmt])
    if rc != 0:
        log.warning("could not list the previews on this daemon (%s)", out.strip()[:160])
        return []
    ended: list[str] = []
    for line in out.splitlines():
        ...
        if len(parts) != 6:
            continue
        _name, state, project, card, expires, pr_url = parts
        ...
        elif pr_url and pr_status is not None:
            try:
                status = pr_status(project, pr_url)
            except Exception as exc:  # noqa: BLE001 — unread is not closed
                log.warning("could not read %s's state (%s) — its preview stays until its time "
                            "is up", pr_url, exc)
                status = "open"
            if status in ("merged", "closed"):
                why = f"its pull request was {status}"
        if why and stop_preview(project, card, why=why):
            ended.append(f"{project} #{card}: {why}")
    return ended
