...
| **`openfactory-knowledge` branch** | Where that bundle is persisted — a dedicated branch in the **client's** repo holding only the bundle, one commit per merge that changes sources. Never `main` (a commit there would fire the client's deploy and push open PRs behind). Disposable: delete it and the next merge republishes. |
