...
| an **`openfactory-knowledge` branch appears** in a driven repository | the knowledge pipeline (ADR-0017) publishing the generated module map — it holds only `knowledge/*.yaml`, never product code, and gains one commit per merge that changes sources | nothing. It is DISPOSABLE: delete it and the next source-changing merge republishes from scratch. To stop it, set `knowledge_map: false` in that project's `.openfactory/project.yaml`. It is never merged into the default branch and never triggers a deploy |
